...
class IRCReplayBot(irc.IRCClient):
...
class UserStatus(resource.Resource):
    ...
    def render_GET(self, request):
        d = self.service.getUsers(self.user)
        d.addCallback(html.escape)
        d.addCallback(lambda m: f'<h1>{self.user} <p>{m}</p>')
        d.addCallback(request.write)
        d.addCallback(lambda _: request.finish())
        return server.NOT_DONE_YET
...
class UserStatusXR(xmlrpc.XMLRPC):
    ...
    def xmlrpc_getUser(self, user):
        return self.service.getUsers(user)
...
internet.ClientService(
    endpoints.clientFromString(reactor, 'tch:irc.freenode.org:6667'),
