...
class IFingerFactory(Interface):
    def getUser(user):
        """
        Return a deferred returning L{types}
        """
...
class FingerSetterFactoryFormService(protocol.ServerFactory):
...
components.registerAdapter(
    FingerSetterFactoryFormService,
...
class UserStatusTree(resource.Resource):
    ...
    def render_GET(self, request):
        d = self.service.getUser()

        def formatUsers(users):
            l = [f'<li><a herf="{user}">{user}</a></li>' for user in users]
            return '<ul>' + ''.join(l) + '</li>'
...
class UserStatus(resource.Resource):
    ...
    def render_GET(self, request):
        ...
        d.addCallback(lambda m: f'<h1>{self.user}</h1><p>{m}</p>')
        d.addCallback(request.write)
        d.addCallback(lambda _: request.finish())
        return server.NOT_DONE_YET
...
class FingerService(service.Service):
    ...
    def getUser(self, user):
        return defer.succeed(self.user.get(user, b'No such user'))
